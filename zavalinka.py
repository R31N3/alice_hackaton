...
def read_data():
    with open("other2/words.json", encoding="utf-8") as file:
        data = json.loads(file.read())
        return data


def read_answers_data():
    with open("other2/alice_answers.json", encoding="utf-8") as file:
        data = json.loads(file.read())
        return data
...
def handle_dialog(request, response, user_storage, database, wrd):
        ...
        if user_storage["asking_name"] and not (database.get_entry(request.user_id) and request.is_new_session):
            if request.is_new_session:
                ...
                response.set_text(aliceSpeakMap("Вас приветствует игра Завалинка. Я буду называть слова, а вы - "
                                                "угадывать и значения. Для того, чтобы следить за твоими успехами, мне"
                                                " потребуется твой никнейм. Как тебя зовут?"))

                response.set_tts(aliceSpeakMap("Вас приветствует игра Завалинка. Я буду называть слова, а вы - "
                                               "угадывать и значения. Для того, чтобы следить за твоими успехами, мне"
                                               " потребуется твой никнейм. Как тебя зовут? Ты "
            ...
            if "name" not in user_storage.keys():
                if not database.get_entry(request.user_id):
                    ...
                    user_storage["name"] = other2.database_module.show_score(database, request.user_id)[1]
            buttons, user_storage = get_suggests(user_storage)
            response.set_buttons(buttons)
            ...
            if not another_flag:
                if user_storage["name"]:
                    choice = random.choice(aliceAnswers["thanksVariations"]) + " " + random.choice(
                        aliceAnswers["helloTextVariations"]).capitalize()
                else:
    ...
    if request.command.lower() in ['давай', 'ладно', 'хорошо', 'ок', 'согласен', 'да',
                                   'не, играть хочу'] and not user_storage.get(request.user_id):
        answered = True
        user_storage[request.user_id] = {"movesLeft": random.randint(15, 25), "text": "Начинаем! ",
                                         "textToSpeech": "Начин+аем!", "words": read_data(), "answer": "", "score": 0}
    ...
    if request.command.lower().strip("?!.") in ['нет', 'не хочется', 'в следующий раз', 'выход', "не хочу", 'выйти']:
        ...
        return response, user_storage
    if "таблица лидер" in request.command.lower().strip("?!.") or request.command.lower().strip("?!.") in [
        "посмотреть"]:
        answered = True
        choice = random.choice(aliceAnswers["resultsShowVariations"])
        results = other2.database_module.show_leaderboard(database, 10)
        resultsText = "\n"
        ...
        # if user_storage["name"]:
        #    resultsText+="А у вас счёт "+str(other2.database_module.show_score(database, request.user_id)[1])+"! И всё таки, " + random.choice(aliceAnswers["helloTextVariations"]).lower()
        # if results:
    ...
    if user_storage.get(request.user_id):
        ...
        if user_storage[request.user_id]["answer"] and not another_flag:
            ...
            if map_answer(request.command).lower() == map_answer(
                    user_storage[request.user_id]["answer"][:len(request.command)]).lower():
                ...
                otvet = random.choice(
                    [["Правильно!", "Пр+авильно!"], ["Отлично!", "Отл+ично!"], ["Молодец!", "Молод+ец!"]])
                ...
                user_storage[request.user_id]["text"] = "Неправильно, это {}. Следующий вопрос: ".format(
                    map_answer(user_storage[request.user_id]["answer"]))
                user_storage[request.user_id]["textToSpeech"] = "Непр+авильно, это {}. Сл+едующий вопр+ос: ".format(
                    map_answer(user_storage[request.user_id]["answer"], True))
                otvet = random.choice(
                    [["Неправильно!", "Непр+авильно!"], ["Неверно!", "Нев+ерно!"], ["Вы ошиблись!", "Вы ош+иблись!"]])
                user_storage[request.user_id]["text"] = otvet[0] + " Это {}. Следующий вопрос: ".format(
                    map_answer(user_storage[request.user_id]["answer"]))
                user_storage[request.user_id]["textToSpeech"] = failure_sound + otvet[
                    1] + " Это {}. Сл+едующий вопр+ос: ".format(
                    map_answer(user_storage[request.user_id]["answer"], True))
        ...
        if user_storage[request.user_id]["movesLeft"] > 0:
            ...
            response.set_tts(
                user_storage[request.user_id]["textToSpeech"] + "{} - это: {}".format(map_answer(word, True), ", ".join(
                    user_storage["suggests"])))
            for e in answers:
                if e[1]:
